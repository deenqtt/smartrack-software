...
function MapRoute({
  ...
  const { map, isLoaded } = useMap();
  const autoId = useId();
  // Ensure ID is safe for MapLibre (remove colons)
  const safeId = (propId ?? autoId).replace(/:/g, "-");
  const sourceId = `route-source-${safeId}`;
  const layerId = `route-layer-${safeId}`;

  // Add source and layer on mount
  useEffect(() => {
    if (!isLoaded || !map) return;

    // Check if source/layer already exists to prevent errors
    if (map.getSource(sourceId)) {
      console.log(`[MapRoute] Source ${sourceId} already exists, skipping add.`);
      return;
    }

    console.log(`[MapRoute] Adding route ${safeId} with coords:`, coordinates);
    ...
    try {
      ...
      console.error(`[MapRoute] Error adding source/layer ${safeId}:`, err);
    }

    return () => {
      // Use a timeout or ensure removal logic is safe
      // Using requestAnimationFrame to unbatch creates smoother removal generally
      if (map) {
        try {
  ...
  useEffect(() => {
    ...
    // Explicitly handle dashArray reset (pass empty array or actual array)
    // MapLibre uses an array for dashes, and we must pass it correctly.
    // Specifying "line-dasharray" reduces the risk of it getting stuck.
