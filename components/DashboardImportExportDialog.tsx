...
interface DashboardImportExportDialogProps {
  open: boolean;
...
export default function DashboardImportExportDialog({
  open,
  onOpenChange,
}: DashboardImportExportDialogProps) {
  ...
  const simulateImport = async () => {
    ...
    try {
      ...
      const result: ImportResult = {
        ...
        widgetsImported: 8,
        errors: [],
        warnings: ["2 duplicate widgets were skipped"]
      };

      setImportResult(result);
      showToast.success('Import completed', `${result.dashboardsImported} dashboards imported successfully`);
      ...
      <Dialog open={open} onOpenChange={handleDialogClose}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader className="text-center">
            ...
            <DialogDescription>
              Import dashboard configurations from a JSON file
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="border-2 border-dashed rounded-lg p-6 text-center">
                  ...
                  <div
                    className={`transition-colors ${
                      dragActive ? 'bg-primary/5 border-primary/50' : 'hover:bg-muted/50'
                ...
                {importResult.success && (
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="bg-muted/50 rounded p-3 text-center">
                      <p className="font-semibold text-lg">{importResult.dashboardsImported}</p>
                      <p className="text-muted-foreground">Dashboards</p>
                    </div>
                    <div className="bg-muted/50 rounded p-3 text-center">
                      <p className="font-semibold text-lg">{importResult.widgetsImported}</p>
