...
interface DailyKeyDetailWidgetConfigModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function DailyKeyDetailWidgetConfigModal({
  isOpen,
  onClose,
  onSave,
  initialConfig,
}: DailyKeyDetailWidgetConfigModalProps) {
  const [title, setTitle] = useState("Daily Key Detail");
  ...
  useEffect(() => {
    if (initialConfig) {
      setTitle(initialConfig.title || "Daily Key Detail");
  ...
  // Load keys when device changes
  useEffect(() => {
    if (selectedDeviceId) {
      loadAvailableKeys(selectedDeviceId);
      setSelectedKey(""); // Reset key when device changes
    } else {
      setAvailableKeys([]);
      setSelectedKey("");
    }
  ...
  const handleSave = () => {
    const config = {
      title: title.trim() || "Daily Key Detail",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Configure Daily Key Detail Widget</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-2">
            ...
            <p className="text-sm text-muted-foreground">
              Select the device that contains the parameter you want to monitor.
            </p>
          </div>
          ...
          <div className="grid gap-2">
            ...
            <Select value={selectedKey} onValueChange={setSelectedKey} disabled={!selectedDeviceId}>
              <SelectTrigger>
            ...
            <p className="text-sm text-muted-foreground">
              Select the specific parameter/key you want to monitor in detail.
            </p>
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={!title.trim() || !selectedDeviceId || !selectedKey}>
            Save Configuration
          </Button>
        </DialogFooter>
      </DialogContent>
