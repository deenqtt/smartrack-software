...
export const PowerGenerationChartWidget = ({ config }: Props) => {
  const { theme } = useTheme();
  const [data, setData] = useState<PowerGenerationData | null>(null);
  const [status, setStatus] = useState<"loading" | "error" | "ok">("loading");
  const [errorMessage, setErrorMessage] = useState("");

  const [selectedView, setSelectedView] = useState<"generation" | "efficiency">("generation");
  ...
  const getSourceIcon = (name: string) => {
    switch (name.toLowerCase()) {
      case 'solar pv': return Sun;
      case 'wind turbine': return Wind;
      case 'diesel generator': return Fuel;
        ...
        series: [
          {
            ...
            emphasis: {
              itemStyle: {
                shadowBlur: 10,
                shadowOffsetX: 0,
                shadowColor: isDark ? "rgba(255, 255, 255, 0.2)" : "rgba(0, 0, 0, 0.5)",
              },
            },
            label: {
              show: true,
  ...
  const getEfficiencyChartOption = () => {
    ...
    const efficiencyData = [
      { name: "System Efficiency", value: data.summary.efficiency, color: "#16a34a" },
    ...
    return (
      <div className="flex gap-3 overflow-x-auto pb-2 mb-4">
        {data.sources.map((source: any, index: number) => {
          ...
          return (
            <div key={index} className={`bg-muted/30 rounded-lg p-3 border flex-1 min-w-[180px] ${
              theme === "dark" ? "border-border/40" : "border-slate-200"
            }`}>
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-xs font-medium ${
                    ...
                    utilization > 50 ? 'text-amber-600' : 'text-red-600'
                  }`}>
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
        <div className={`text-center p-2 rounded-lg ${
          theme === "dark" ? "bg-muted/20 border border-border/40" : "bg-slate-50 border border-slate-200"
        }`}>
          <div className={`text-xs font-medium ${
            theme === "dark" ? "text-slate-400" : "text-slate-600"
          }`}>Total Generation</div>
    ...
    if (status === "loading") {
      return (
        <div className="flex flex-col items-center justify-center gap-3 h-full">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
          <p className={`text-sm ${theme === "dark" ? "text-slate-400" : "text-slate-600"}`}>Loading generation data...</p>
        </div>
      );
    }
    ...
    if (!data) {
      return (
        <div className="flex flex-col items-center justify-center gap-3 h-full">
          ...
          <p className={`text-sm ${
            theme === "dark" ? "text-slate-400" : "text-slate-600"
          }`}>No generation data available</p>
        </div>
      );
    }
        ...
        <div className={`flex items-center gap-3 border-b pb-3 mb-3 ${
          ...
          <div>
            ...
            <p className={`text-xs ${
              ...
              {selectedView === "generation" ? "Renewable energy mix" :
