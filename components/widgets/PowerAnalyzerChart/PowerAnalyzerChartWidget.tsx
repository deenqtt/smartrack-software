  ...
  useEffect(() => {
    ...
    const loadData = async (isInitialLoad = true) => {
      try {
        ...
        const averageVoltage = safeAverage(phases.map(p => p.voltage));
        const averageCurrent = safeAverage(phases.map(p => p.current));
        const systemPF = safeAverage(phases.map(p => p.pf));
        const totalTHD = safeAverage(phases.map(p => p.thd));

        const powerData: RealPowerAnalysisData = {
          summary: { totalPower, averageVoltage, averageCurrent, systemPF, totalTHD },
          phases,
          harmonics: Array.from({ length: 15 }, (_, i) => ({ harmonic: i + 1, value: i === 0 ? 100 : 0 })),
          quality: {
            voltageUnbalance: averageVoltage
              ? Math.max(
                  ...phases.map((phase) =>
              ...
              : 0,
            currentUnbalance: averageCurrent
              ? Math.max(
                  ...phases.map((phase) =>
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          ...
          <div
            className={`text-2xl font-bold ${theme === "dark" ? "text-slate-100" : "text-slate-900"
              }`}
          >
          ...
          >
            {quality.voltageUnbalance < 2
              ? "Good"
              : quality.voltageUnbalance < 3
                ? "Fair"
                : "Poor"}
          </div>
        </div>
        ...
        >
          ...
          >
            {quality.currentUnbalance < 10
              ? "Good"
              : quality.currentUnbalance < 20
                ? "Fair"
                : "Poor"}
          </div>
        </div>
