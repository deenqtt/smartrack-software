...
export const LineChartConfigModal = ({
  isOpen,
  onClose,
  onSave,
  initialConfig,
  ...
  const handleAddConfig = (configName: string) => {
    if (!selectedConfigNames.includes(configName) && selectedConfigNames.length < 5) {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-4xl">
        <DialogHeader className="px-6 pt-6">
          ...
          <DialogDescription>
            Configure a multi-series line chart to compare trends across multiple data sources.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-2">
              <Label>Selected Data Sources ({selectedConfigNames.length}/5)</Label>
          ...
          <div className="space-y-4">
            ...
            <div className="mt-6 p-4 bg-muted/50 rounded-lg">
              ...
              <div className="text-xs text-muted-foreground space-y-1">
                ...
                <div><strong>Legend:</strong> {showLegend ? "Yes" : "No"}</div>
                <div><strong>Dual Axis:</strong> {dualAxis ? "Yes" : "No"}</div>
                <div><strong>Smooth Lines:</strong> {smoothLines ? "Yes" : "No"}</div>
              </div>
            </div>

            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
              <p className="text-xs text-blue-700 dark:text-blue-300">
                <strong>Tip:</strong> Select 2-4 data sources for optimal chart readability.
                Too many series may make the chart difficult to interpret.
              </p>
            </div>
          </div>
        </div>
